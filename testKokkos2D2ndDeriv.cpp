...
int main(int argc, char *argv[]) {
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    Kokkos::View<double *[2], MemSpace> res(
        "2nd deriv result", offsetMirror(offsetMirror.extent(0) - 1));
    ...
    for (int i = 0; i < evalAt.size(); i++) {
      ...
      if (xDiff > EPSILON || yDiff > EPSILON) {
        std::cout << "Test " << i + 1 << " failed, eval at: " << evalAt[i]
                  << std::endl;
        std::cout << "Difference: x = " << xDiff << " y = " << derivY
                  << std::endl;
        std::cout << "SERIAL 2nd deriv: x = " << derivX << ", " << derivY
                  << std::endl;
        std::cout << "KOKKOS 2nd deriv: x = " << mvRes(i, 0) << ", "
                  << mvRes(i, 1) << std::endl;
        retVal = 1;
      }
    }
